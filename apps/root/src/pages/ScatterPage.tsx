...
const SCATTER_URL =
	source === "local" ? "/scatter_latest.json" : "https://cdn.veille.safecoffi.app/scatter/latest.json";

const UMAP1D_URL =
	source === "local" ? "/scatter_umap1d_latest.json" : "https://cdn.veille.safecoffi.app/scatter/umap1d_latest.json";
...
export default function ScatterPage() {
	...
	return (
		<div className="space-y-4 px-10 py-5">
			<h1 className="text-foreground text-lg font-semibold">Clusters</h1>
			...
			{tab === "connected" && (
				<>
					...
					{scatter.error && (
						<div className="flex h-64 items-center justify-center">
							<p className="text-red-500">Erreur : {(scatter.error as Error).message}</p>
						</div>
					)}
					{scatter.data && (
						<>
			...
			{tab === "umap1d" && (
				<>
					...
					{umap1d.error && (
						<div className="flex h-64 items-center justify-center">
							<p className="text-red-500">Erreur : {(umap1d.error as Error).message}</p>
						</div>
					)}
					{umap1d.data && (
						<>
