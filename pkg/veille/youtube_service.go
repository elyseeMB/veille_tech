...
func FetchYouTube(ctx context.Context, conn *db.PostgresConnection) error {
	...
	results := make(chan Result, len(repository.YouTubeChannels))
	var wg sync.WaitGroup

	for _, channel := range repository.YouTubeChannels {
		wg.Add(1)
		go func(ch repository.YouTubeChannelConfig) {
			defer wg.Done()
			...
			if ytRes.err != nil {
				slog.Warn("youtube playlist fetch failed", "channel", ch.Name, "error", ytRes.err)
			...
			if playlist.Error != nil {
				slog.Warn("youtube api error", "channel", ch.Name, "error", playlist.Error.Message)
				results <- Result{videos: nil}
				return
			}
	...
	var allVideos []coredata.VideoDB
	...
	newVideos, err := repository.FilterNewVideos(conn, unique)
	if err != nil {
		return fmt.Errorf("filter new videos failed: %w", err)
	}

	if err := repository.InsertVideos(conn, newVideos); err != nil {
		return fmt.Errorf("insert videos failed: %w", err)
	}

	slog.Info("youtube sync completed", "new", len(newVideos), "skipped", len(unique)-len(newVideos))
	return nil
}
