...
func GetGraph(conn *db.PostgresConnection, date string) (*coredata.GraphResponse, error) {
	...
	var nodes []coredata.GraphNode
	for rows.Next() {
		var n coredata.GraphNode
	...
	entityRows, err := conn.Pool.Query(
		context.Background(),
	...
	var edges []coredata.GraphEdge

	linkRows, err := conn.Pool.Query(
		context.Background(),
	...
	return &coredata.GraphResponse{Nodes: nodes, Edges: edges}, nil
}
